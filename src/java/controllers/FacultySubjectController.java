...
public class FacultySubjectController implements Serializable {
    ...
    private List<FacultySubject> l = new ArrayList();
    private List<FacultySubject> p;
    ...
    public List<FacultySubject> getPractical() {
        return p;
    }

    public String prepareDetails(short batch) {
        ...
        for (Subject s : subjectList) {
            ...
            if (temp != null && !l.contains(temp)) {
                l.add(temp);
            }
        }

        for (Subject s : subjectList) {
            FacultySubject temp = getFacade().findByBatch(fs, s, batch);
            if (temp != null && !p.contains(temp)) {
                p.add(temp);
            }
        }
    ...
    public FacultySubject getFacultySubject(java.lang.Integer id) {
    ...
    public static class FacultySubjectControllerConverter implements Converter {
        ...
        public Object getAsObject(FacesContext facesContext, UIComponent component, String value) {
            ...
            return controller.getFacultySubject(getKey(value));
        }
